...
Q3PopupMenu * CMaskedLineEdit::createPopupMenu ( const QPoint & pos )
{Q3PopupMenu *pQPopupMenu = new Q3PopupMenu(0, "MyCustomPopupMenu" );
 if (pQPopupMenu==0)    return 0;
 QPoint qpoint(pos);
 m_IsSel = getSelection ( &m_debMix, &m_endMix );
 pQPopupMenu->insertItem( tr("Annu&ler         \t Ctrl+Z"),         this, SLOT( undo()),         Qt::CTRL+Qt::Key_Z );
 pQPopupMenu->insertItem( tr("&Refaire         \t Ctrl+Y"),         this, SLOT( redo()),         Qt::CTRL+Qt::Key_Y );
 pQPopupMenu->insertSeparator ( ) ;
 pQPopupMenu->insertItem( tr("&Couper          \tCtrl+X"),          this, SLOT( cut()),          Qt::CTRL+Qt::Key_X );
 pQPopupMenu->insertItem( tr("C&opier          \tCtrl+C"),          this, SLOT( copy()),         Qt::CTRL+Qt::Key_C );
 pQPopupMenu->insertItem( tr("Cop&ie étendue   \tShift+Ctrl+C"),    this, SLOT( extendedCopy()), Qt::SHIFT+Qt::CTRL+Qt::Key_Y );
 pQPopupMenu->insertItem( tr("C&oller          \tCtrl+V"),          this, SLOT( paste()),        Qt::CTRL+Qt::Key_P );
 //pQPopupMenu->insertItem( tr("E&ffacer      \tCtrl+Del"),        this, SLOT( clear()),  CTRL+Key_R );

 pQPopupMenu->insertSeparator ( ) ;
 pQPopupMenu->insertItem( tr("&Tout sélectionner \tCtrl+A"),        this, SLOT( selectAll()),    Qt::CTRL+Qt::Key_A );
