...
*/
QString CGestIni::_Param_WriteParam(        QString *pQstr, const QString &section, const QString &variable,
...
{long section_length = section.length();
 if (pQstr == 0        )    return QString::null;
 if (pQstr->length()==0  && section.length())   *pQstr = QString("[")+section+"]\n" ;
 ...
 {switch (pt[0])
     ...
     */
     case '[': //....................debut de section: tester si c'est celle recherchee ...................
          ...
          // len_section = end - deb;
          // QString toCompare = QString::fromLatin1 ( deb, len_section );
          if ( section == QString::fromLatin1 ( deb, end - deb ))   // SI c'est la section recherchee
              ...
              while (*pt && *pt != '[')                                                // tant que pas fin de zone de section (debut d'une autre)
                 {
                  ...
                  if (*pt=='=')
                     {end = pt;
                      ...
                      if (variable == QString::fromLatin1 ( deb, end - deb ))
                         {deb_lgn = deb;
                          end_lgn = end;
                          ...
                          if (val1.length())     result +=        val1;
                          if (val2.length())     result +=  " , " + val2;
                          if (val3.length())     result +=  " , " + val3;
                          if (val4.length())     result +=  " , " + val4;
                          if (val5.length())     result +=  " , " + val5;
                          if (val6.length())     result +=  " , " + val6;
                          if (val7.length())     result +=  " , " + val7;
                          if (val8.length())     result +=  " , " + val8;
                          if (val9.length())     result +=  " , " + val9;
                          if (val10.length())    result +=  " , " + val10;
               ...
               tmp = tmp + "\n";
               tmp = tmp + "  " + variable + " = ";
               if (val1.length())     tmp  +=          val1;
               if (val2.length())     tmp  +=  " , " + val2;
               if (val3.length())     tmp  +=  " , " + val3;
               if (val4.length())     tmp  +=  " , " + val4;
               if (val5.length())     tmp  +=  " , " + val5;
               if (val6.length())     tmp  +=  " , " + val6;
               if (val7.length())     tmp  +=  " , " + val7;
               if (val8.length())     tmp  +=  " , " + val8;
               if (val9.length())     tmp  +=  " , " + val9;
               if (val10.length())    tmp  +=  " , " + val10;
               tmp     = tmp + "\n";
               pos     = pt-txt;
...
/*
QString CGestIni::Param_WriteParam(   const QString *pQstr, const char *section, const char  *variable,
...
*/
/*
QString CGestIni::Param_ReadParam(  const QString &txt, const QString &section, const QString &variable,
...
{ int len_param = txt.length();
  if (len_param==0)   return QObject::tr("Syntax error: empty parameter file");
 ...
 {
    if ( txt[pt] =='\r'||txt[pt] =='\n'||txt[pt]== ';')
       { pt =  CGestIni::gotoNextStartLine(txt, pt);
       }
    else if ( txt[pt] == '/')
       { if (pt+1<len_param )
           { if (txt[pt+1]=='/') pt  =  CGestIni::gotoNextStartLine(txt, pt);
             else                pt += 2;
           }
         else
           { ++pt;        // on devrait etre le dernier caractere ...
           }
       }
    else if ( txt[pt] == '[')  //....................debut de section: tester si c'est celle recherchee ...................
       { ++pt;
          pt  = CGestIni::gotoNextNotBlank(txt, pt);
          deb = pt;
          while (pt<len_param && txt[pt] != ']') ++pt;                                        // chercher delimiteur de fin de nom de section
          end = pt;
          while (end>deb && (txt[end-1]==' '||txt[end-1]=='\t'|| txt[end-1]=='\a')) --end;    // enlever espaces de fin
          if (section == txt.mid(deb,end-deb))
             {pt = CGestIni::gotoNextNotBlank (txt, pt);
              while (pt<len_param && txt[pt] != '[')              // tant que pas fin de zone de section (debut d'une autre ou fin)
                 {
                  while (pt<len_param && (txt[pt]==' '||txt[pt]=='\t'||txt[pt]=='\a'))  ++pt ;                                             // sauter espaces de debut de ligne
                  if  (txt[pt]==';' || txt[pt]=='\n'|| txt[pt] =='\r'||txt[pt]=='\a')
                      { pt = gotoNextStartLine(pt) ;                   // sauter les commentaires et retour ligne en allant ligne suivante
                        while (pt<len_param && (txt[pt]==' '||txt[pt]=='\t'||txt[pt]=='\a'))  ++pt ;  // ressauter espaces de debut de ligne
                      }
                  else if (txt[pt]=='/'&& pt+1<len_param && txt[pt+1] =='/')
                      { pt = gotoNextStartLine(pt);
                        while (pt<len_param && (txt[pt]==' '||txt[pt]=='\t'||txt[pt]=='\a'))  ++pt ;
                      }                                       }
                  deb = pt;                       // on devrait etre au debut du nom d'une variable
                  //while (*pt && *pt!='=')  ++pt;  // le signe '=' marque la fin de zone du nom de la variable
                  while (pt<len_param && txt[pt]!='=' && txt[pt]!='[')
                        {if (txt[pt]=='\\' && pt+1<len_param) pt += 2;
                         else ++pt;
                        }
                  if (pt==len_param||txt[pt]=='[') return QObject::tr("Sign = not found in section : '%1'', for requested variable : '%2'").arg(section, variable) ;
                  end = pt;
                  while (end>deb && (end[-1]==' '||end[-1]=='\t'|| end[-1]=='\a')) end-- ;   // enlever espaces de fin du nom de la vaiable
                  len_variable = end - deb;
                  if ((long)strlen(variable)==len_variable && strncmp(variable, deb, len_variable)==0)   // SI c'est la variable recherchee
...
{while (pos<outParam.length() && (outParam[pos]=='\r' || outParam[pos]=='\n' || outParam[pos]==' ' || outParam[pos]=='\t' || outParam[pos]=='\a')) ++pos;
 return pos;
}
//----------------------------------------- gotoNextBlank ---------------------------------------------
...
//-------------------------- listDirectory -------------------------------------------
/*! \brief non documente */
QStringList CGestIni::listDirectory(QStringList &dirList, QString start_dir, const QString &filterExt_in/*=""*/, const QString &filterName/*=""*/, const QString &sep/*=";"*/, bool listWithoutExt/*=false*/)
{   QStringList    ret;
...
{   QStringList dirList;
    return CGestIni::listDirectory(dirList, start_dir, filterExt_in, filterName, sep, listWithoutExt);
}
//-----------------------------------------------------  PassWordEncode --------------------------------------------------
...
//----------------------------------------- Copy_Dir ---------------------------------------------
void CGestIni::Copy_Dir(QString src_Dir,  QString dest_Dir)
{
    //........................ explorer la source .......................................
    ...
    {   fi = *it;
        ...
        if      ( fi.isDir() )
                ...
                {    CGestIni::Copy_File(src_Dir + "/" + fname, dest_Dir);
                }
        ++it;
    }
}
...
QString CGestIni::Copy_File(const QString &fileSrc, const QString &pathDst, QString *file_name /*=0*/)
{
//................. choisir le fichier image src a inserer .................................................
  ...
  if ( ! (pathDst == QFileInfo( *pfileSrc ).absolutePath() && QFileInfo( *pfileDst ).fileName() == QFileInfo( *pfileSrc ).fileName()) )
     {char *buffer = new char[32000];
      ...
      QFile::remove ( dstF );                               // eventuellement effacer un  du m̻me nom
