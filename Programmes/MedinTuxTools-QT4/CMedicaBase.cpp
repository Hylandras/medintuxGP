...
int CMedicaBase::Datasemp_MakeAllIndexProduit(QProgressBar *pQProgressBar)
{   int nb(0);
    int ret;
    m_Progress = 0;
    m_Run      = 1;
...
//----------------------------------------------- Datasemp_MakeIndex ---------------------------------------------------------------
int CMedicaBase::Datasemp_MakeIndex(int table_type, QProgressBar *pQProgressBar)
{ QString    tableName;
  QString    requete;
  QString    code_prod;
  QString    pk_t4B;
 ...
 if (Medica_CreateTableIndexProduit(tableName)==0)                                  return -1;
  ...
  if (query.isActive())
        ...
        {    //............... recuperer pk de la t4b correspondant au code produit de la t00 .......................
             ...
             if ( cur.canInsert ())
                {//............... si OK on recupere le buffer et on le charge avec les données ......................
                 ...
                 if (!cur.insert()) OutSQL_error(cur, "Datasemp_MakeIndex(): Error cur.insert", requete);   // ecrire le buffer
                }
             else
               {OutSQL_error(cur, "Datasemp_MakeIndex(): Error cur.canInsert", requete);
                return -2;
               }
             nb++;
        } //end while (pSqlQuery->next())
    ...
    {OutSQL_error(query, "Datasemp_MakeIndex(): Error select t00", requete);
    }
  return nb;
}
...
//-------------------------------------------------- OutSQL_error -----------------------------------------------
int CMedicaBase::OutSQL_error( const QSqlQuery &cur, const char *messFunc /*=0*/, const char *requete /*=0*/)
