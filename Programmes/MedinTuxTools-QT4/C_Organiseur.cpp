...
#include "../../MedinTuxTools/Calendar/Dlg_Calendar.h"
...
C_Organiseur::C_Organiseur(EVNT_LIST *pEvnmtList, RUBREC_LIST *pRubList, const QString &dossPk, QWidget * parent /* = 0 */, const char * name /*= 0*/, WFlags f /*= 0*/)
 :m_pEvnmtList(pEvnmtList), m_pRubList(pRubList), m_DossPk(dossPk) , Form_Organiseur(parent, name, f)
{   QString value;
    m_log = "";
    m_IsDragedFile         = FALSE;
    m_PathProfiles         = G_pCApp->m_PathAppli+"Ressources/Vigie/";
    m_ClasseursAllVisibles = 0;
    m_LastImgPath          = "";
    //m_FiltreHierarchList.append("Classeur sejour/kine");
    ...
    m_ListView_MonitorPatient = new C_VigieListView( m_splitter, "m_ListView_MonitorPatient" );
  ...
  Form_OrganiseurLayout->addWidget( m_splitter, 1, 0 );
  ...
  //loadProfile(m_PathProfiles+"Profiles/VigieProfile.txt");
  //............. charger les profils liés à l'utilisateur .......................
  QString xml = "";
  READ_USER_PARAM( "Dock_Vigie", tr("Profil"), &xml);
  if (xml.length())
     {CGestIni::retrieveStringForParam(xml);
      SetProfile(xml);
     }
  xml = "";
  READ_USER_PARAM( "Dock_Vigie", tr("Filtres"), &xml);
  if (xml.length())
     {CGestIni::retrieveStringForParam(xml);
      SetFilter(xml);
     }
  ...
  READ_USER_PARAM("Dock_Vigie", tr("HierarchFilter"), &xml);
  xml = xml.upper();
  ...
  xml = "";
  READ_USER_PARAM("Dock_Vigie", tr("DefaultHierarchFilter"), &xml);
  if (xml.length())
  ...
  xml = "";
  READ_USER_PARAM( "Dock_Vigie", tr("ListViewMode"), &xml);
  if (xml.length())
  ...
  //.............. activer le patient en cours ........................................
  CGestIni::Param_ReadParam(DRTUX_USER_PARAM, "Dock_Vigie", "FontSize", &value );
  if (value.length())
     {QFont f = font();
...
void C_Organiseur::saveUserProfil()
{
 ...
 xml = GetProfile();
 CGestIni::formatStringForParam(xml);
 WRITE_USER_PARAM( "Dock_Vigie", tr("Profil"), xml);
 G_pCApp->m_pCMoteurBase->Param_SavParam(G_pCApp->m_User, &DRTUX_USER_PARAM);
}
...
{ QFont f = font();
  ...
  setFont( f );
  Set_ZoomFontSize(s);
  WRITE_USER_PARAM  (&DRTUX_USER_PARAM, "Dock_Vigie", "FontSize", QString::number(s));
  UPDATE_USER_PARAM ( G_pCApp->m_User );
}
...
{ QFont f = font();
  ...
  setFont( f );
  Set_ZoomFontSize(9);
  WRITE_USER_PARAM  (&DRTUX_USER_PARAM, "Dock_Vigie", "FontSize", QString::number(9));
  UPDATE_USER_PARAM ( G_pCApp->m_User );
}
...
{ if (pQListViewItem==0 || pCRubRecord==0) return;
  ...
  pQListViewItem->setPixmap ( DT_Deb-1,   Theme::getIcon("16x16/calendrierDeb.png"));
  pQListViewItem->setPixmap ( DT_Fin+1,   Theme::getIcon("16x16/calendrierFin.png"));
...
{if (date.length()==0) return QString::null;
 QString text = C_TokenInterpret::toNum(date);
...
{ QString name = pQListViewItem->text(0);
  OpClProfiles::Iterator it = m_OpenCloseProfiles.find(name);
  if (it == m_OpenCloseProfiles.end()) return;         // pas de profile pour ce classeur cassos
...
//----------------------------------------------- loadProfile ------------------------------------------------------------------------
void C_Organiseur::loadProfile(const QString &path)
{QString xml_p = "";     // profil
 ...
 if (xml_f.length()) SetFilter  (xml_f);
 if (xml_p.length()) SetProfile (xml_p);
}

//----------------------------------------------- saveProfile ------------------------------------------------------------------------
void  C_Organiseur::saveProfile(const QString &path)
{CGestIni::Param_UpdateToDisk(path, GetProfile()+"\n"+GetFilter() );
}

//----------------------------------------------- GetProfile ------------------------------------------------------------------------
QString  C_Organiseur::GetProfile()
{if ( m_OpenCloseProfiles.isEmpty () ) return QString::null;
 QString xml = "<!DOCTYPE CVigie>\n<CVigie version=\"0.0\" stdsetdef=\"1\">\n";
 OpClProfiles::Iterator it;
 for ( it           = m_OpenCloseProfiles.begin(); it != m_OpenCloseProfiles.end(); ++it )
    {QString str   = it.data();
     QString value = "";
...
{//......... virer le dernier .................
 ...
 for ( QStringList::Iterator it = listClasseurs.begin(); it != listClasseurs.end(); ++it )
     { int          pos_deb      = 0;
      ...
      m_OpenCloseProfiles[classeur_name] = classeur;
     }
 return m_OpenCloseProfiles.count();
}
...
{ QString profile = "";
  ...
  OpClProfiles::Iterator it = m_OpenCloseProfiles.find(name);
  if (it == m_OpenCloseProfiles.end())
     {profile = " name=\""; profile += name + "\">\n";
     }
 else
     {profile = m_OpenCloseProfiles[name];
     }
 ...
 if (deb == -1)       // le tag n'a pas ete trouve donc on suppose qu'il n'existe pas on le cree
    {int    pos  = profile.findRev("</Classeur>");
     ...
     profile                  += QString("\n     <")+xmltag+">"+ value + "</"+xmltag+">\n  </Classeur>\n";
     m_OpenCloseProfiles[name] = profile;
     return;
    }
 deb += xmltag.length() + 2;
...
{OpClProfiles::Iterator it = m_OpenCloseProfiles.find(name);
 if (it == m_OpenCloseProfiles.end()) return QString::null;
 QString value;
 if ( CGenTools::GetXMLValue(m_OpenCloseProfiles[name], QString("<")+xmltag+">", value, 0) != -1 ) return value;
 return QString::null;
}
...
*/
QString C_Organiseur::Append_Doc_In_Hierarchie(CRubRecord* pCRubRecordToAdd, QString hierarch)
{if (m_IsDragedFile) return QString::null;
 ...
 if (m_pFindHierarchEvnt==0)      //>>> ............ PAS DE HIERARCHIE TROUVEE POUR CET ELEMENT ...................
    ...
    {C_DragQListViewItem* pC_DragQListViewItem = GetListViewItem_FromPkEvnt(m_pFindHierarchEvnt->m_Pk);
     ...
     if (pC_DragQListViewItem)
        { if (m_ClasseursAllVisibles==0 && isInVisible=="1")
          ...
          if (isUnique=="1" && Find_Evnt_WhithSameName_InFratrie(m_pRubList, m_pFindHierarchEvnt, pCRubRecordToAdd->m_Libelle))
             {QString libelle = pCRubRecordToAdd->m_Libelle; //libelle.replace(" ","&nbsp;");
              //QMessageBox::warning( this, "La vigie vous informe ...",
              //                     tr("<u>Opération impossible</u> :<br>"
              //                         "Un classeur unique correspondants à ce document : <b>") + libelle + tr("</b>  existe déjà."
...
{ QHeader *pQHeader = m_ListView_MonitorPatient->header();
  ...
  OpClProfiles::Iterator it = m_OpenCloseProfiles.find(pQListViewItem->text(0));
  if (it == m_OpenCloseProfiles.end()) return;         // pas de profile pour ce classeur cassos
  QString dataToExamine = it.data();
  QString   value       = "";
...
{ int h=0; int m=0; int s=0; int ms=0; int len = heure.length();
  QString str = C_TokenInterpret::toNum(heure);
  len         = str.length();
...
{QStringList menuList;
 ...
 QString evntPk      = pC_DragQListViewItem->GET_PK_EVNT();        // sauver les meubles pour retrouver ensuite le bon item
  if (c==DT_Deb-1)
    ...
    {C_Dlg_Pendule* dlg = new C_Dlg_Pendule( this);
     ...
     QTime t_deb  = hhmmssToQTime(txt_time );    if ( !t_deb.isValid () || txt_time.length()==0) { t_deb = QTime::currentTime(); }
     ...
     txt_date     = txt_time = "";
     pos          = txt_dateTime.find(tr("à")); if (pos != -1) {txt_date=txt_dateTime.left(pos); txt_time = txt_dateTime.mid(pos+1).stripWhiteSpace();}
     QTime t_fin  = hhmmssToQTime(txt_time );     // ne pas corriger car si fin invalide (par exemple vide) on peut quand meme rentrer la date de debut
    ...
    {C_Dlg_Pendule* dlg = new C_Dlg_Pendule( this);
     ...
     txt_date     = txt_time = "";
     pos          =  txt_dateTime.find(tr("à")); if (pos != -1) {txt_date=txt_dateTime.left(pos); txt_time = txt_dateTime.mid(pos+1).stripWhiteSpace();}
     QTime t_fin  = hhmmssToQTime(txt_time );    if ( !t_fin.isValid ()|| txt_time.length()==0) { if (t_deb.isValid()) t_fin = t_deb; else t_fin = QTime::currentTime(); }
    ...
    {QString   listeDeroulante  = "";
     ...
     OpClProfiles::Iterator it  = m_OpenCloseProfiles.find(text);
     if (it != m_OpenCloseProfiles.end())
        { QString dataToExamine     = it.data();
 ...
 if (menuItem==itemDel)
    ...
    {if (m_pEvnmtList->FirstStateFindInParents( pCRubEvntPair, TO_DELETE))
     ...
     pCRubEvntPair->removeState(TO_DELETE);
     m_pEvnmtList->Evnmt_Remove_StateOn_AllFils(pCRubEvntPair->m_Pk, TO_DELETE);
    }
    ...
    { //..... l'item .......
      ...
      //.. les fils.........
      Evnmt_Set_StateOn_AllFilsAndDoc(pCRubEvntPair->m_Pk, C_Organiseur::toDelete);
    }
...
{ QString        listName   = "";
  ...
  OpClProfiles::Iterator it = m_OpenCloseProfiles.find(classeurName);
  if (it != m_OpenCloseProfiles.end())
     {QString dataToExamine = it.data();
      QString   value       = "";
 ...
 dlg->initDialog(G_pCApp->m_PathGlossaire, G_pCApp->m_PathAppli);
...
void C_Organiseur::pushButtonDateDeb_clicked()
    {   Dlg_Calendar          *dlg  = new Dlg_Calendar(this, "Calendar_Dial", FALSE);
        ...
        Dlg_DateInit(dlg);
        QString text = C_TokenInterpret::toNum(mle_deb->text());
        int jour, mois, annee;
        jour  = text.mid(0,2).toInt();
        mois  = text.mid(2,2).toInt();
        annee = text.mid(4,4).toInt();
...
{       Dlg_Calendar          *dlg  = new Dlg_Calendar(this, "Calendar_Dial", FALSE);
        ...
        Dlg_DateInit(dlg);
        QString text = C_TokenInterpret::toNum(mle_fin->text());
        int jour, mois, annee;
        jour  = text.mid(0,2).toInt();
        mois  = text.mid(2,2).toInt();
        annee = text.mid(4,4).toInt();
                     ...
                     while (query.next())
                           {
                            ...
                            if (query.value( 18).toString()==m_DossPk)
                               ...
                               { pQListViewItem = 0;
                                 ...
                                 GetEvenementsFilsFromSQL(  pQListViewItem, crpair, headerRequete, rubRecord.m_Libelle);  // rehercher les enfants
                                }
     ...
     {while (query.next())
            {
               ...
               if (ok && pQListViewItem)
                  {C_DragQListViewItem *pQListViewItem_Fils = new C_DragQListViewItem (pQListViewItem);
                   if (pQListViewItem_Fils) SetListViewItemFromRecord( pQListViewItem_Fils, &rubRecord , &crPair);
