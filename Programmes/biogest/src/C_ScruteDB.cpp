...
//----------------------------------------- C_ScruteDB -------------------------------
C_ScruteDB::C_ScruteDB(C_Dlg_BioGest *pC_Dlg_BioGest, QObject *parent) :
    QObject(parent)
{
 ...
 m_pC_Dlg_BioGest      = pC_Dlg_BioGest;
 m_pC_ScruteDB_Thread  = new C_ScruteDB_Thread(this);
 m_ScrutationTimer     = new QTimer(this);
 m_PatientPk_CPatient_Map.clear();
 setScrutationInterval (qMax(15,(CApp::pCApp()->readUniqueParam("biogest session" , "DataBase scrutation").toInt())));
...
//--------------------------------------- setService ----------------------------------
void C_ScruteDB::setService(const  QString &pk, const QString &service)
{CPatient patient = m_PatientPk_CPatient_Map[pk];
 patient.m_Service = service;
 m_PatientPk_CPatient_Map[pk] = patient;
}

//--------------------------------------- traiteLaBaseDeDonnee ----------------------------------
// appelé par le thread
void C_ScruteDB::traiteLaBaseDeDonnee()
{   if (CApp::pCApp()->getDB() == 0)
       {  CApp::pCApp()->outMessage(tr("°       <font color=#ff0000><b> C_ScruteDB::traiteLaBaseDeDonnee() CApp::m_pC_BaseBioGest is not instancied, may be database server is unreachable. </b></font>."));    // <font color=#ff5400><b>%1</b></font>
          return;
       }
    int                 nbNewPatients = 0;
    int                 nbChgPatients = 0;
    ...
    QList<QTreeWidgetItem *>  listItemSelected = m_pC_Dlg_BioGest->m_pGUI->treeWidget_Patients->selectedItems();
    if (listItemSelected.size())
       {QTreeWidgetItem *pQTreeWidgetItem = listItemSelected[0];
        id_patientSelected = pQTreeWidgetItem->text(7);
       }
    if (m_pC_ScruteDB_Thread->m_clearQTreeWidget) m_pC_Dlg_BioGest->m_pGUI->treeWidget_Patients->clear();
    //............ on va cherche la liste des patients en cours .........................
    int limit = m_pC_Dlg_BioGest->Slot_fill_CPatientMap(m_PatientPk_CPatient_Map);
    ...
    m_pC_Dlg_BioGest->setStatusLabeMessage(str);
    ...
    {//.......... voir si l'item de la liste patient est présent dans le treeWidget (on matche sur le Pk) .............
     QList<QTreeWidgetItem *> itemList = m_pC_Dlg_BioGest->m_pGUI->treeWidget_Patients->findItems ( patient.getPkRecord(), Qt::MatchExactly, 7 );
     ...
     //.......... si cet item est non present dans le treeWidget alors le rajouter ................
        ...
        {
          pQTreeWidgetItem = new QTreeWidgetItem(m_pC_Dlg_BioGest->m_pGUI->treeWidget_Patients);
          if (pQTreeWidgetItem)
    ...
    if (id_patientSelected.length())
      {listItemSelected = m_pC_Dlg_BioGest->m_pGUI->treeWidget_Patients->findItems(id_patientSelected, Qt::MatchExactly,7);
       if (listItemSelected.size())
          {m_pC_Dlg_BioGest->m_pGUI->treeWidget_Patients->setCurrentItem(listItemSelected[0]);
           listItemSelected[0]->setSelected(true);
           m_pC_Dlg_BioGest->Slot_treeWidget_Patients_itemClicked( listItemSelected[0],0);
          }
      }
...
{setScrutationInterval (CApp::pCApp()->readUniqueParam("biogest session" , "DataBase scrutation").toInt());
 scruteEtTraiteLaBaseDeDonnee();
