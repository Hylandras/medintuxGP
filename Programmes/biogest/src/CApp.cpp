...
{   QString pathTemplate = "";
    m_pC_Utils_Html = 0;
    //......................... recuperer le template d'integration html ..................................
    m_HprimTempl      = "";  // paranoiac attitude
    m_pathTemplate    = "";  // idem
    m_pathTemp        = "";
    m_pCCoolPopup     = 0;
    m_CCoolPopupTempo = 2000;          /*!< temps de persistance du menu ?  l'?©cran en microsecondes */
    m_pC_BaseBioGest  = 0;

    //................ le template d'integration ..........................
    setPathTemplate( readUniqueParam("biogest session", "Repertoire des templates") );
    pathTemplate =  getPathTemplates()+getCurrentTemplateFileName();
    if (!QFile::exists ( pathTemplate))
       {QMessageBox::warning(0, mui_name,
                             tr("Le template  : <br><b>'%1'</b><br>n'a pas été trouvé. ").arg(pathTemplate),
                             QMessageBox::Cancel
                            );
        return;
      }
    G_pCApp      =    this;
    //.............................. recuperer le template d'integration hprim ..........................................
    m_HprimTempl = CGestIni::Param_UpdateFromDisk(getPathTemplates()+getCurrentTemplateFileName());

    //.............................. initialiser la base de donnees ..........................................
    createDb();     // si ok m_pC_BaseBioGest est instancie sinon est sur zero
    setPathTemp(readUniqueParam("biogest session", "Repertoire temporaire"));
...
//------------------------------------ createDb -------------------------------
/*! \brief cree et instancie la base de donnees C_BaseBioGest *m_pC_BaseBioGest
 */
bool CApp::createDb()
{   //.............................. recuperer config de la base ..........................................
    QString paramCfgData = CGestIni::Param_UpdateFromDisk(m_PathAppli + m_NameAppli+"Bases.cfg");
    if (m_pC_BaseBioGest) delete m_pC_BaseBioGest;
    m_pC_BaseBioGest = 0;
    if (m_pC_BaseBioGest==0)
       {m_pC_BaseBioGest      = new C_BaseBioGest(m_IniParam, paramCfgData, this);
        if (m_pC_BaseBioGest == 0)
           {qDebug () << tr("CApp::createDb() echec de new C_BaseBioGest");
            return false;
           }
        if (m_pC_BaseBioGest->getLastError().length())
           {qDebug() << m_pC_BaseBioGest->getLastError();
            QMessageBox::warning(0, m_NameAppli,
                             tr("CApp::createDb() Echec de l'ouverture de la base : \n%1.").arg(m_pC_BaseBioGest->getLastError()),
                             QMessageBox::Cancel
                            );
            delete m_pC_BaseBioGest;
            m_pC_BaseBioGest = 0;
            return false;
           }
      }
...
bool CApp::connectDb()
{if (m_pC_BaseBioGest)
    {if ( m_pC_BaseBioGest->reConnect(m_IniParam) )
        { return true;
        }
     else
        { delete m_pC_BaseBioGest;
 ...
 */
QStringList CApp::getProfilsList(const QString &varToFind, const QString &section /* = "biogest session" */)
...
{return (readUniqueParam("biogest session", "LogFTP").toLower()[0]=='o'); // TODO lire une variable plutot qu'un fichier ...
}
//------------------------------------ isFtpDaemonEnabled -------------------------------
/*! \brief retourne si le demon ftp doit etre active en lisant le fichier de configuration
 */
bool CApp::isFtpDaemonEnabled()
{return (readUniqueParam("biogest session", "Ftp daemon enabled").toLower()[0]=='o');
}
//-------------------------- setFtpDaemonEnabled -------------------------------------------
...
{QString etat = (state==Qt::Checked)?"ok":"no";
 writeParam("biogest session", "Ftp daemon enabled", etat.toAscii());
 updateIniParamToDisk();
}
...
{return (readUniqueParam("biogest session", "Just patients with results").toLower()[0]=='o');
}
//------------------------------------ getResultsLimit -------------------------------
/*! \brief retourne le nombre limite de resultats a afficher
 */
QString CApp::getResultsLimit()
{return readUniqueParam("biogest session", "Limiter le nombre de resultats");
}
//-------------------------- setFtpDaemonEnabled -------------------------------------------
...
{QString etat = (state==Qt::Checked)?"ok":"no";
 writeParam("biogest session", "Just patients with results", etat.toAscii());
 updateIniParamToDisk();
}
//------------------------------------ isDateFilterEnabled -------------------------------
...
{return (readUniqueParam("biogest session", "date filter").toLower()[0]=='o');
}
...
{QString etat = (state==Qt::Checked)?"ok":"no";
 writeParam("biogest session", "date filter", etat.toAscii());
 updateIniParamToDisk();
}
//-------------------------- setdefaultService -------------------------------------------
...
{writeParam("biogest session", "Service", serv.toAscii());
 updateIniParamToDisk();
}
//-------------------------- setdefaultService -------------------------------------------
...
{return readUniqueParam("biogest session", "Service");
}
//-------------------------- getdefaultFtp -------------------------------------------
/*! \brief retourne le ftp par defaut
 */
QString CApp::getdefaultFtp()
{return readLine("biogest session", "ServeurFTP");
}
//-------------------------- setdefaultService -------------------------------------------
...
*/
C_BaseBioGest* CApp::getDB()
{return m_pC_BaseBioGest;
}
//-------------------------- initUtilsHtml -------------------------------------------
...
//------------------------------------ getTemplate -------------------------------
/*! \brief retourne le template en cours pour presentation des donnees
 *  \return QString le template en cours pour presentation des donnees
 */
 QString CApp::getTemplate()
 {return m_HprimTempl;
 }
 //------------------------------------ setTemplate -------------------------------
 /*! \brief positionne le template en cours pour presentation des données sur la valeur donnee en entree
  *  \param const QString &templat valeur que prendra le template en cours
  */
  void CApp::setTemplate(const QString &templat)
  { if (templat.length()==0) return;
    m_HprimTempl  =   templat;
  }
  //------------------------------------ getCurrentTemplateFileName -------------------------------
  /*! \brief retourne le nom du fichier template en cours d'utilisation doit etre trouve dans le repertoire des templates
   */
   QString CApp::getCurrentTemplateFileName()
   {QString ret =  readUniqueParam("biogest session", "Template en cours");
    return  ret;
   }
   //------------------------------------ getCurrentTemplateFileName -------------------------------
   /*! \brief positionne le nom du fichier template en cours d'utilisation doit etre trouve dans le repertoire des templates
    */
    void CApp::setCurrentTemplateFileName(const QString &file)
    { writeParam("biogest session", "Template en cours", file.toAscii());
    }
  //------------------------------------ getPathTemplates -------------------------------
  /*! \brief retourne le chemin du template en cours pour presentation des donnees
   *  \return QString  le chemin du template en cours pour presentation des donnees
   */
  QString CApp::getPathTemplates()
  {return m_pathTemplate;
  }
  //------------------------------------ setPathTemplate -------------------------------
  /*! \brief positionne le chemin du template en cours pour presentation des données sur la valeur donnee en entree
   *  \param const QString &pathTemplat valeur que prendra le chemin du template en cours
   */
  void CApp::setPathTemplate(const QString &pathTemplat)
  { if (pathTemplat.length()==0) return;
    m_pathTemplate  =   pathTemplat;
    if (QDir::isRelativePath(m_pathTemplate))  m_pathTemplate = QDir::cleanPath ( m_pathTemplate.prepend(pathAppli()) );
    if (!m_pathTemplate.endsWith('/')) m_pathTemplate += "/";
  }
  //------------------------------------ setPathTemp -------------------------------
