...
//------------------------------------ C_Dlg_SigEntrees -------------------------------
C_Dlg_SigEntrees::C_Dlg_SigEntrees(QWidget *parent) :
    QMainWindow(parent),
...
{   QString        defStr = "";
    ...
    m_pGUI->setupUi(this);
    //----------------------------------------------------SIG
    CApp::pCApp()->G_p_Dlg_SigEntrees = m_pGUI;
    ...
    m_pQLabel->show();
    m_pQLabel->setText(tr("Welcome on sigentrees application"));
    ...
    m_pGUI->checkBox_traceEnabled->setCheckState(CApp::pCApp()->readUniqueParam("sigentrees session", "LogFTP").toLower()[0]!='n'?Qt::Checked:Qt::Unchecked);
    ...
    m_pGUI->checkBox_ftp_distantsDelete->setCheckState(CApp::pCApp()->readUniqueParam("sigentrees session", "EffacementDistant").toLower()[0]!='n'?Qt::Checked:Qt::Unchecked);
    ...
    m_pGUI->checkBox_ftp_localsDelete->setCheckState(CApp::pCApp()->readUniqueParam("sigentrees session", "EffacementLocal").toLower()[0]!='n'?Qt::Checked:Qt::Unchecked);

    m_pGUI->lineEdit_ftp_scrutation_time->setText(CApp::pCApp()->readUniqueParam("sigentrees session", "Ftp scrutation"));
    m_pGUI->lineEdit_ip_scrutation_time->setText(CApp::pCApp()->readUniqueParam("sigentrees session", "DataBase scrutation"));
    ...
    CApp::pCApp()->replaceComboboxListInParam(m_pGUI->comboBox_ftp_fav,  "sigentrees session", "_ServeurFTP");
...
//    connect( m_pGUI->treeWidget_Patients ,        SIGNAL(customContextMenuRequested( const QPoint &)),                      this, SLOT(Slot_treeWidget_Patients_customContextMenuRequested (const QPoint &)) );
    ...
    comboBox_ftp_fav
    */
    // Masquer les trucs et machins inutil�s en attendant de voir ....
    m_pGUI->comboBox_Service->hide();
...
void C_Dlg_SigEntrees::Slot_lauch_C_ScruteFTP()
{ showMinimized ();
...
//------------------------ ~C_Dlg_SigEntrees ---------------------------------------
C_Dlg_SigEntrees::~C_Dlg_SigEntrees()
{if (m_pC_ScruteFTP) delete m_pC_ScruteFTP;
 if (m_pC_ScruteDB)  delete m_pC_ScruteDB;
}

//------------------------ Slot_retranslateUi ---------------------------------------
void C_Dlg_SigEntrees::Slot_retranslateUi()
{
}

//------------------------ closeEvent ---------------------------------------
void C_Dlg_SigEntrees::closeEvent(QCloseEvent *event)
...
//------------------------ Slot_onQuit ---------------------------------------
void C_Dlg_SigEntrees::Slot_onQuit()
{
}

//------------------------ Slot_m_HelpIndicator_clicked ---------------------------------------
void C_Dlg_SigEntrees::Slot_m_HelpIndicator_clicked()
{
    ...
    argList << "sigentrees";                                                     // 1  nom du module
...
void C_Dlg_SigEntrees::Slot_timerStateIndicator_clicked()
...
void C_Dlg_SigEntrees::setStatusLabeMessage(const QString & mess)
{m_pQLabel->setText(mess);
}

//------------------------ Slot_FTP_status ---------------------------------------
void C_Dlg_SigEntrees::Slot_FTP_status(int status)
...
void C_Dlg_SigEntrees::Slot_pushButton_logErase_Clicked()
{m_pGUI->textEdit_Monitor->clear();
}

//------------------------ Slot_comboBox_ftp_fav_Activated ---------------------------------------
void C_Dlg_SigEntrees::Slot_comboBox_ftp_fav_Activated(const QString &str )
{   QStringList paramList = str.split(",");
    ...
    CApp::pCApp()->writeParam("sigentrees session", "ServeurFTP",paramList[0].toAscii(),paramList[1].toAscii(),paramList[2].toAscii(),password.toAscii(),paramList[4].toAscii());
    CApp::pCApp()->writeParam("sigentrees session", "Ftp scrutation",m_pGUI->lineEdit_ftp_scrutation_time->text().toAscii());
    CApp::pCApp()->updateIniParamToDisk();
    if (m_pC_ScruteFTP && CApp::pCApp()->isFtpDaemonEnabled()) m_pC_ScruteFTP->startScrutation();
}
//------------------------ Slot_comboBox_ip_fav_Activated ---------------------------------------
//       driver               shema                 log                    passw                hostna�e              port
void C_Dlg_SigEntrees::Slot_comboBox_ip_fav_Activated(const QString &str )
{   QStringList paramList = str.split(",");
    ...
    CApp::pCApp()->writeParam("sigentrees session", "DataBase scrutation",m_pGUI->lineEdit_ip_scrutation_time->text().toAscii());
    CApp::pCApp()->updateIniParamToDisk();
    if (m_pC_ScruteDB)
...
void C_Dlg_SigEntrees::Slot_lineEdit_resultsLimit_editingFinished()
{CApp::pCApp()->writeParam("sigentrees session", "Limiter le nombre de resultats",m_pGUI->lineEdit_resultsLimit->text().toAscii());
 CApp::pCApp()->updateIniParamToDisk();
}
//------------------------ Slot_lineEdit_ftp_scrutation_time_editingFinished ---------------------------------------
void C_Dlg_SigEntrees::Slot_lineEdit_ftp_scrutation_time_editingFinished()
{CApp::pCApp()->writeParam("sigentrees session", "Ftp scrutation",m_pGUI->lineEdit_ftp_scrutation_time->text().toAscii());
...
void C_Dlg_SigEntrees::Slot_lineEdit_ip_scrutation_time_editingFinished()
{CApp::pCApp()->writeParam("sigentrees session", "DataBase scrutation",m_pGUI->lineEdit_ip_scrutation_time->text().toAscii());
...
void C_Dlg_SigEntrees::Slot_pushButton_playHprim_Clicked()
{QStringList filesList = QFileDialog::getOpenFileNames (this,
 ...
 for (int i = 0; i < filesList.size(); ++i)
     {   localFileName = filesList.at(i);
         ...
         CApp::pCApp()->outMessage(tr("C_Dlg_SigEntrees::Slot_pushButton_playHprim_Clicked() analyse finished <font color=#ff5400><b>%1</b></font> code : %2 ").arg(localFileName, mess));
     }  // end for (int i = 0; i < filesList.size(); ++i)

}
//------------------------ Slot_dateEdit_Fin_dateChanged ---------------------------------------
void C_Dlg_SigEntrees::Slot_dateEdit_Fin_dateChanged ( const QDate &  )
{Slot_fillPatientList();
}
//------------------------ Slot_dateEdit_Deb_dateChanged ---------------------------------------
void C_Dlg_SigEntrees::Slot_dateEdit_Deb_dateChanged ( const QDate &  )
{Slot_fillPatientList();
}

//------------------------ Slot_checkBox_justPatientsWithResults_stateChanged ---------------------------------------
void C_Dlg_SigEntrees::Slot_checkBox_justPatientsWithResults_stateChanged( int state)
{CApp::pCApp()->setJustPatientsWithResults( state );
 Slot_fillPatientList();
}
//------------------------ Slot_checkBox_filtrerDates_stateChanged ---------------------------------------
void C_Dlg_SigEntrees::Slot_checkBox_filtrerDates_stateChanged( int state)
{CApp::pCApp()->setDateFilterEnabled( state );
 Slot_fillPatientList();
}

//------------------------ Slot_lineEdit_PatientSearch_textChanged ---------------------------------------
void C_Dlg_SigEntrees::Slot_lineEdit_PatientSearch_textChanged(const QString &)
{Slot_fillPatientList();
}

//------------------------ Slot_pushButton_ip_connect_Clicked ---------------------------------------
void C_Dlg_SigEntrees::Slot_pushButton_ip_connect_Clicked()
{

    CApp::pCApp()->outMessage(tr("================= C_Dlg_SigEntrees::Slot_pushButton_ip_connect_Clicked() ================================"));
 //..................................................... si m_pC_ScruteDB est non instancie c'est probablement .....................................................
 //                                                      lie au fait que CApp::m_pC_BaseSigEntrees n'a pu etre cree
 ...
 if (CApp::pCApp()->getDB()==0 )
    { if (CApp::pCApp()->controlDb()==false)   // on tente de creer la base de donn�es
         CApp::pCApp()->outMessage(tr(".           C_Dlg_SigEntrees::Slot_pushButton_ip_connect_Clicked() <font color=#ff0000><b>Error: database is always not instancied. </b></font>"));
     else
         CApp::pCApp()->outMessage(tr(".           C_Dlg_SigEntrees::Slot_pushButton_ip_connect_Clicked() OK database is connected"));
    }
 else
...
//------------------------ Slot_fillPatientList ---------------------------------------
void C_Dlg_SigEntrees::Slot_fillPatientList()
...
int C_Dlg_SigEntrees::Slot_fill_CPatientMap(QMap <QString,CPatient> &mapPk_Cpatient)
...
void C_Dlg_SigEntrees::Slot_comboBox_Service_Activated(const QString &text )
{Slot_fillPatientList();
 CApp::pCApp()->setdefaultService(text);
}

//---------------------------------------------- Slot_pushButtonDateDeb_Clicked -----------------------------------------------------------------------
void C_Dlg_SigEntrees::Slot_pushButtonDateDeb_Clicked ()
{
...
void C_Dlg_SigEntrees::Slot_pushButtonDateFin_Clicked ()
{
    //............... recuperer la date de ce calendrier .................................................
...
void C_Dlg_SigEntrees::Slot_checkBox_ftp_distantsDelete_stateChanged( int state)
{   QString etat = (state==Qt::Checked)?"ok":"no";
    CApp::pCApp()->writeParam("sigentrees session", "EffacementDistant", etat.toAscii());
    CApp::pCApp()->updateIniParamToDisk();
...
void C_Dlg_SigEntrees::Slot_checkBox_ftp_localsDelete_stateChanged( int state)
{   QString etat = (state==Qt::Checked)?"ok":"no";
    CApp::pCApp()->writeParam("sigentrees session", "EffacementLocal", etat.toAscii());
    CApp::pCApp()->updateIniParamToDisk();
    if (m_pC_ScruteFTP) m_pC_ScruteFTP->setLocalsFileBeErasedState((state==Qt::Checked));
}

//------------------------ Slot_checkBox_traceEnabled_stateChanged ---------------------------------------
void C_Dlg_SigEntrees::Slot_checkBox_traceEnabled_stateChanged( int state)
{m_pGUI->textEdit_Monitor->setEnabled(state==Qt::Checked);
 QString etat = (state==Qt::Checked)?"ok":"no";
 CApp::pCApp()->writeParam("sigentrees session", "LogFTP", etat.toAscii());
 CApp::pCApp()->updateIniParamToDisk();
}

//------------------------ Slot_checkBox_ftp_daemon_enabled_stateChanged ---------------------------------------
void C_Dlg_SigEntrees::Slot_checkBox_ftp_daemon_enabled_stateChanged( int state)
{
...
void C_Dlg_SigEntrees::Slot_DB_AnalyseFinished(const QString &message)
{ if (message.length())
     {CApp::pCApp()->outMessage(message);
      if (isMinimized()) CApp::pCApp()->CouCou(C_Utils_Log::removeHtmlTag(tr("Nouveaux r�sultats\ndisponibles")),Theme::getPath()+"PopupLabo.png",-1);
     }
  else
...
//------------------------ Slot_CouCouDestroy ---------------------------------------
void C_Dlg_SigEntrees::Slot_CouCouDestroy()
{ showNormal();
}

//------------------------ Slot_FilesAnalyseFinished ---------------------------------------
void C_Dlg_SigEntrees::Slot_FilesAnalyseFinished()
{
}

//------------------------ Slot_treeWidget_Patients_itemClicked ---------------------------------------
void C_Dlg_SigEntrees::Slot_treeWidget_Patients_itemClicked( QTreeWidgetItem *pQTreeWidgetItem, int)
{
 if (pQTreeWidgetItem==0) return;
 getFicHprimResultsFromPatientPk(pQTreeWidgetItem->text(7));
// if ( m_pC_ScruteDB ) pQTreeWidgetItem->setIcon(0, m_pC_ScruteDB->m_IsAlreadyView);

}

//------------------------ Slot_treeWidget_Patients_itemDoubleClicked ---------------------------------------
void C_Dlg_SigEntrees::Slot_treeWidget_Patients_itemDoubleClicked( QTreeWidgetItem *pQTreeWidgetItem , int)
{   Q_UNUSED(pQTreeWidgetItem)
    /*
    if (pQTreeWidgetItem==0) return;
    QString pathLastCopy= CApp::pCApp()->readUniqueParam("sigentrees session", "PathLastCopy");
...
void C_Dlg_SigEntrees::Slot_treeWidget_Patients_itemSelectionChanged ()
{   /*
...
//------------------------ getFicHprimResultsFromPatientPk ------------------------
void C_Dlg_SigEntrees::getFicHprimResultsFromPatientPk(QString PKentree)
{

    m_pGUI->textEdit_hprim->setText(CApp::pCApp()->getDB()->read_Blob_Hprim(PKentree )) ;
}

/*
//------------------------ Slot_treeWidget_Patients_customContextMenuRequested ---------------------------------------
void C_Dlg_SigEntrees::Slot_treeWidget_Patients_customContextMenuRequested (const QPoint &)
