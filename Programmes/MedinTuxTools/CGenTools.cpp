...
{    pathFile   = pathFile.stripWhiteSpace();
     int   pos  = pathFile.find("$Glossaire");
     if (  pos != -1 )
        { if (pathFile[pos+10]=='/' && pathGlossaire[0]=='/') pathFile = pathFile.remove(pos+10,1);
          pathFile = pathFile.replace("$Glossaire",pathGlossaire);
        }
